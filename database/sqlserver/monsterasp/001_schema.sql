/*
    GestorProveedores - Esquema para MonsterASP.NET
    Alineado con la BD real local: (localdb)\MSSQLLocalDB / GESTORPROVEEDORES

    Ejecutar conectado a la base creada en MonsterASP (no master).
    Alternativa recomendada: restaurar GESTORPROVEEDORES_local.bak
*/
...
BEGIN
    ...
    (
        ...
        CONSTRAINT CK_Usuarios_Rol CHECK (Rol IN (N'superusuario', N'administrador', N'solicitante', N'auxiliar', N'analista', N'aprobador', N'contable')),
        CONSTRAINT CK_Usuarios_EmpresaSegunRol CHECK
        (
            (Rol IN (N'solicitante', N'aprobador') AND EmpresaId IS NOT NULL)
            OR (Rol IN (N'superusuario', N'administrador', N'auxiliar', N'analista', N'contable') AND EmpresaId IS NULL)
        )
    );
END;
GO
...
BEGIN
    ...
    ALTER TABLE dbo.Usuarios WITH CHECK ADD CONSTRAINT CK_Usuarios_Rol
        CHECK (Rol IN (N'superusuario', N'administrador', N'solicitante', N'auxiliar', N'analista', N'aprobador', N'contable'));
